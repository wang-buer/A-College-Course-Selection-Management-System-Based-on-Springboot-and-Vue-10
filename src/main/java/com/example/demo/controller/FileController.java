...
//    ${server.port}
    ...
    public Result<?> upload(@RequestBody MultipartFile file) throws IOException {
        ...
        String flag = IdUtil.fastSimpleUUID(); // 定义文件的唯一标识（前缀）,避免重名文件的冲突
        String rootFilePath = System.getProperty("user.dir") + "/files/" + flag + "_" + originalFilename;  // 获取文件路径
        FileUtil.writeBytes(file.getBytes(), rootFilePath);  // 使用Hutool工具类的方法，把文件写入到上传的路径
